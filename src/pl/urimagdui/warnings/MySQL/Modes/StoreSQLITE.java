...
import pl.urimagdui.warnings.MySQL.Callback;
import pl.urimagdui.warnings.MySQL.Store;
import pl.urimagdui.warnings.Utils.Logger;
import pl.urimagdui.warnings.Utils.Timming;
...
public class StoreSQLITE implements Store {
    ...
    public boolean connect() {
        ...
        try {
            ...
            this.conn = DriverManager.getConnection("jdbc:sqlite:" + Main.getPlugin().getDataFolder() + File.separator + Config.DATABASE_SQLITE_NAME);
            Logger.info("Uzywanie lokalnych plikow!");
            return true;
        }
        catch (ClassNotFoundException e) {
            Logger.warning("Problem z polaczeniem sie z lokalnymi plikami!", "Blad: " + e.getMessage());
            e.printStackTrace();
        }
        catch (SQLException e2) {
            Logger.warning("Problem z polaczeniem sie z lokalnymi plikami", "Blad: " + e2.getMessage());
            e2.printStackTrace();
        }
        return false;
    }
